...
tb_self <- read_delim(file = nucmer_file_self,
                 delim = "\t",
                 col_names = T)
# Columns:
# tID
...
# subset
coords1 <- tb_self %>% filter(tID==nucmer_ref , qID==nucmer_ref , identity > min_iden , match > min_len )
...
maxWidth <- unit.pmax(gt0_widths, gt1_widths, gt2_widths, gt3_widths , gt5_widths , gt6_widths , gt7_widths )
...
layout <- rbind(1,2,3,4,5,6,7)

g <- arrangeGrob( gt0, gt1 , gt5 , gt6 , gt7 , gt2 , gt3, layout_matrix=layout , heights=c(.25, .25, .05 , .05 , .05 , .1 , .25) )
